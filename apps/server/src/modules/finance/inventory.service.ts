...
export class InventoryService {
  ...
  async consume(itemId: string, quantity: number) {
    ...
    await this.db.transaction(async (tx) => {
      ...
      this.checkAndTriggerInventoryAlert(itemId).catch((err) => {
        this.logger.error(
          `Failed to trigger inventory alert for ${itemId}:`,
          err,
        );
      });
    });
  ...
  async getInventoryItems(userId: string, ledgerId: string) {
    const items = await this.db
      .select()
      .from(schema.inventoryItems)
      .where(
  ...
  async recordWaste(
    ...
    const batches = await this.db
      .select()
      .from(schema.inventoryBatches)
      .where(eq(schema.inventoryBatches.itemId, item.id))
      .all();
    const avgUnitPrice =
      batches.length > 0
        ? batches.reduce((sum, b) => sum + (b.unitPrice || 0), 0) /
          batches.length
        : 0;
    ...
    return await this.db.transaction(async (tx) => {
      ...
      for (const batch of sortedBatches) {
        ...
        const consumedFromBatch = Math.min(
        ...
        await tx
          .update(schema.inventoryBatches)
          .set({
            remainingQuantity: sql`${schema.inventoryBatches.remainingQuantity} - ${consumedFromBatch}`,
          })
          .where(eq(schema.inventoryBatches.id, batch.id));
