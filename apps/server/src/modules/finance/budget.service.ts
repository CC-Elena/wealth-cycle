...
export class BudgetService {
  ...
  async getBudgetPlans(ledgerId?: string) {
    ...
    for (const plan of plans) {
      ...
      const linkedCategories = this.db.select()
      ...
      if (categoryIds.length > 0) {
        ...
        const start = plan.periodStart || new Date(new Date().getFullYear(), new Date().getMonth(), 1);
        const end = plan.periodEnd || new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0);
  ...
  async createBudgetPlan(data: CreateBudgetPlan) {
    ...
    if (data.categoryIds && data.categoryIds.length > 0) {
      for (const catId of data.categoryIds) {
        this.db.insert(schema.budgetCategories).values({
          budgetId: id,
          categoryId: catId,
        }).run();
      }
  ...
  async updateBudgetPlan(id: string, data: UpdateBudgetPlan) {
    ...
    if (data.categoryIds) {
      // Re-sync categories
      this.db.delete(schema.budgetCategories)
