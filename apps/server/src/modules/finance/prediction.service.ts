...
export class PredictionService {
  ...
  async getBudgetPredictions(ledgerId: string) {
    const budgets = await this.db
      ...
      .where(
        and(
          eq(schema.budgetPlans.ledgerId, ledgerId),
          eq(schema.budgetPlans.isActive, true),
        ),
      );
